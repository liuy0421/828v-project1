function [out_w, out_b,out_y,out_x] = Project1main_allStates(states)
close all
%% read data
...
ind = ~isfinite(A(:,2)) |  ~isfinite(A(:,3)) | ~isfinite(A(:,4)) ...
    | ~isfinite(A(:,5)) | ~isfinite(A(:,6)) | ~isfinite(A(:,7)) ...
    | ~isfinite(A(:,8)) | ~isfinite(A(:,9));
A(ind,:) = [];
%% select CA, OR, WA, NJ, NY counties
if strcmp(states,'CA-WA')
    ind = (A(:,1)>=6000 & A(:,1)<=6999) ... % ...  %CA
        | (A(:,1)>=53000 & A(:,1)<=53999);        %WA
elseif strcmp(states,'CA-WA-NJ-NY-OR')
    ind = (A(:,1)>=6000 & A(:,1)<=6999) ... % ...  %CA
        | (A(:,1)>=53000 & A(:,1)<=53999) ...        %WA
...
lighting gouraud
alpha(0.3);
saveas(gcf, strcat(['sinewton_plane_' states '.png']))
%%
figure;
hold on;
grid;
...
saveas(gcf, strcat(['sinewton_f_' states '.png']))
%%
figure;
hold on;
grid;
...
saveas(gcf, strcat(['sinewton_fnorm_' states '.png']))
end
%%
